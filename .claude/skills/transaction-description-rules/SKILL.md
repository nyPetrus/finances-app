...
- **Three ways to bulk-categorize transactions from history**, all in
  ...
  (see `PITFALLS.md`). `autoCategorizeFromHistory()` (the "Auto-categorize
  from history" sparkles button) goes a different direction: for every
  description that appears on an already-categorized transaction but has
  *no* explicit mapping yet, it infers one (the majority category/class
  combo for that exact description, `equal_to` only) via an upsert keyed on
  the `(user_id, description)` primary key, then calls
  `syncMappedDescriptions()` to apply the combined rule set. It never
  overwrites a description that already has an explicit mapping, even a
  `starts_with`/`contains` one that hasn't matched anything yet.
